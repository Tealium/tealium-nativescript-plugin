...
class TealiumWrapper {
  ...
  companion object {
    ...
    var consentStatus: String
      ...
      set(value) {
        when (value) {
          "consented" -> tealium?.consentManager?.userConsentStatus = ConsentStatus.CONSENTED
    ...
    private fun checkAndApplyOptions(config: TealiumConfig, options: Map<String, Any>) {
      options.forEach { (key, value) ->
        when (key) {
          ...
          "overrideLibrarySettingsURL" -> config.overrideLibrarySettingsUrl = value.toString()
          "overrideTagManagementURL" -> config.overrideTagManagementUrl = value.toString()
          "deepLinkTrackingEnabled" -> config.deepLinkTrackingEnabled = value.toString().toBoolean()
